/* this.drawDropDown()
Put that at the bottom of either the drawPlot() or in the constructor of GapPlot*/
...
class GapPlot {
    ...
    drawPlot() {
        ...
        svgGroup.append('text')
            //.text('Someone was here') //update this later using an id in update plot
            .attr('id', 'x-axisLabel')
            .attr('text-anchor', 'middle')
            .attr('dominant-baseline', 'middle')
            .attr('x', ((this.width+this.margin.left+this.margin.right )/ 2) )
            .attr('y', this.height+(this.margin.bottom)/2+this.margin.top)

        svgGroup.append('text')
            //.text('Someone else was over here ') //update this later using an id in update plot
            .attr('id', 'y-axisLabel')
            .attr('transform', 'rotate(270)')
            .attr('text-anchor', 'middle')
            .attr('x', (-1*this.height/2)) // When you rotate it, you have to switch your x and y
            .attr('y', ((this.margin.left)/4 )) //((this.height+this.margin.top)/2)

        // Now add the year text in the background
        svgGroup.append('text')
            .text('2000') //update this later using an id in update plot
            .attr('class', 'activeYear-background')
            .attr('id', 'activeYear-display')
            .attr('text-anchor', 'middle')
            .attr('x', (this.margin.left+this.width/4)     ) // When you rotate it, you have to switch your x and y
            .attr('y', ((this.height/4 ))) //((this.height+this.margin.top)/2)
...
// attr('transform', 'rotate(90)')
    ...
    updatePlot(activeYear, xIndicator, yIndicator, circleSizeIndicator) {
        xIndicator = xIndicator || 'child-mortality';
        yIndicator = yIndicator || 'gdp';
        circleSizeIndicator = circleSizeIndicator || 'population';
        /*if (xIndicator) {
       ...
       this.drawDropDown(xIndicator, yIndicator, circleSizeIndicator)
        ...
        let that = this; 
        //console.log(d3.max(Object.values(that.encyclopedia), d => d[circleSizeIndicator] ? d[circleSizeIndicator][activeYear] : 0))
        let circleSizer = function (d) { 
            let cScale = d3.scaleSqrt()
            .range([3, 20])
            .domain([0, d3.max(Object.values(that.encyclopedia), d => d[circleSizeIndicator] ? d[circleSizeIndicator][activeYear] : 0)]);
            return d.circleSize ? cScale(d.circleSize) : 3;
        };
        let xScale = d3.scaleLinear()
            .domain([0, d3.max(Object.values(this.encyclopedia).map(d => d[xIndicator] ? d[xIndicator][activeYear] : 0))])
            .range([0, this.width-this.margin.right - this.margin.left])

        let yScale = d3.scaleLinear()
            .domain([0, d3.max(Object.values(this.encyclopedia).map(d => d[yIndicator] ? d[yIndicator][activeYear] : 0))])
            .range([this.height, 0])

        //let xScaler = function (d) {
        /*let xScale = d => {
            if (!d) return 0;
            return d3.scaleLinear()
                .domain([0, d3.max(data.encyclopedia, d => d[xIndicator] ? d[xIndicator][activeYear] : 0)])
                .range([0, this.width])
        }
        let yScale = d => {
            if (!d) return 0;
            return d3.scaleLinear()
                .domain([0, d3.max(data.encyclopedia, d => d[yIndicator] ? d[yIndicator][activeYear] : 0)])
                .range([0, this.height])
        }*/

        let x_axis = d3.axisBottom(xScale).ticks(10)
        d3.select("#xAxis").call(x_axis)
        let y_axis = d3.axisLeft(yScale).ticks(10)
        d3.select("#yAxis").call(y_axis)
        // Add the scatter plot info 
        

        let plotDataArray = Object.entries(this.encyclopedia).map(([key, item]) => 
            new PlotData(item.gdp.country, // All countries have a GEO
                item[xIndicator] ? xScale(item[xIndicator][activeYear]) : 0,
                item[yIndicator] ? yScale(item[yIndicator][activeYear]) : this.height,
                key,
                item['population'] ? item['population']['region'] : '',
                item[circleSizeIndicator] ? item[circleSizeIndicator][activeYear] : null
            )
        )

        let scatterCircles = d3.select('#plotSection').selectAll('circle').data(plotDataArray, d => d.id) //This grabs the values for each entry rather than taking the entire dictionary
        ...
        newCircles.append('title').text(d => d.country)
        ...
        newCircles.on('click', d => {
            this.updateCountry(d.id)
        })
        newCircles.on('mouseover', d =>  {
            d3.select('#chart-view .tooltip')
        ...
        // Add the text for the axes 
        d3.select('#x-axisLabel').text(this.data[xIndicator][0].indicator_name)
        d3.select('#y-axisLabel').text(this.data[yIndicator][0].indicator_name)
        this.drawLegend(d3.min(Object.values(that.encyclopedia), d => d[circleSizeIndicator] ? d[circleSizeIndicator][activeYear] : 0), d3.max(Object.values(that.encyclopedia), d => d[circleSizeIndicator] ? d[circleSizeIndicator][activeYear] : 0))
   ...
   /**
    ...
    drawDropDown(xIndicator, yIndicator, circleSizeIndicator) {
        ...
        dropC.on('change', function (d, i) {
            ...
            that.circleSizeIndicator = cValue //////////// THis line 
        ...
        dropX.on('change', function (d, i) {
            ...
            that.updatePlot(that.activeYear, xValue, yValue, cValue); 
            console.log(xValue)
            //this.xIndicator = xValue
            that.xIndicator = xValue ////// This line 
            //xIndicator = xValue
        });
        ...
        dropY.on('change', function (d, i) {
            ...
            let cValue = dropC.node().value;
            that.updatePlot(that.activeYear, xValue, yValue, cValue);
            //this.yIndicator = yValue ///// This line
            that.yIndicator = yValue ///// None of these seem to update the parent element
            //yIndicator = yValue
    ...
    drawYearBar() {
        ...
        */
        ...
        yearSlider.on('input', function () {
            ...
            /*let currentDict = {}
            currentDict.xIndicator = d3.select('#dropdown_x').select('dropdown-content').select('select').node.value 
            currentDict.yIndicator = d3.select('#dropdown_y').select('dropdown-content').select('select').node.value
            currentDict.circleSizeIndicator = d3.select('#dropdown_c').select('dropdown-content').select('select').node.value*/
            d3.select('#activeYear-display').text(currentYear)
            //that.updatePlot(this.value, currentDict.xIndicator, currentDict.yIndicator, currentDict.circleSizeIndicator)
            //TODO - your code goes here -
    ...
    drawLegend(min, max) {
        ...
        circleGroup.attr('transform', (d, i) => 'translate(' + ((i * (3 * scale(d))) + 0) + ', 25)');
    ...
    updateHighlightClick(activeCountry) {
        ...
        */
        ...
        this.clearHighlight()
        if (activeCountry){
            d3.select('#plot-'+activeCountry) // these ones are lower case
                .classed('selected-country', true)
            //tooltipRender(this.data)
        }
        

        /*
        this.clearHighlight();
        let T_circles = d3.select('.plot-svg').selectAll('circle)
        let F_count = T_circles.flter(T_circles =>
            T_circles.id.toUpperCase() == activeCountry.toUpperCase()
            )
        var F_region = F_count.datum().region
        var hregion = T_circles.filter()
        */
     ...
     */
    clearHighlight() {
        d3.select('#scatter-plot .selected-country').classed('selected-country', false)
        // ******* TODO: PART 3*******
